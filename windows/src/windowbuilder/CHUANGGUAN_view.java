...
public class CHUANGGUAN_view {
	...
	private void initialize() {
		...
		CHUANGGUAN_view = new JFrame();//框架的相关设置
		...
		CHUANGGUAN_view.setTitle("练习模式");
		CHUANGGUAN_view.setBackground(Color.WHITE);//框架背景
		CHUANGGUAN_view.setBounds(100, 100, 450, 300);
		CHUANGGUAN_view.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		CHUANGGUAN_view.getContentPane().setLayout(null);//将框架的布局管理器关掉
		...
		CHUANGGUAN_view.getContentPane().add(Button_1);//将按钮加入框架
		
		
		
		JButton Button_2 = new JButton("重置");//重置的按钮
		Button_2.addActionListener(new ActionListener() {//重置的按钮事件
			public void actionPerformed(ActionEvent e) {
		...
		CHUANGGUAN_view.getContentPane().add(Button_2);//将按钮加入框架
		...
		CHUANGGUAN_view.addComponentListener(new ComponentAdapter() {//拖动窗口监听，按钮、背景图随窗口大小自适应
            public void componentResized(ComponentEvent e) {  
               
            	
            	int width=CHUANGGUAN_view.getWidth();//获取窗口宽度
                int height=CHUANGGUAN_view.getHeight();//获取窗口高度
